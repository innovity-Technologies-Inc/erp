   ...
   var total_stock = $("#total_stock").val();
    $('#checkListStockList').DataTable({ 
           ...
           'lengthMenu':[[10, 25, 50,100,250,500, total_stock], [10, 25, 50,100,250,500, "All"]],
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': base_url + 'report/report/paysenz_checkStocklist',
                data:{
...
function customerdelete(id) {
    ...
    if (r == true) {
        $.ajax({
            url: base_url + "customer/customer/paysenz_delete/"+id,
            type: "GET",
            success: function (r) {
               toastr["success"](r);
                setTimeout(function () {
   ...
   var base_url  = $('#base_url').val();
    var mydatatable = $('#Credit_customerList').DataTable({ 
           ...
           'lengthMenu':[[10, 25, 50,100,250,500, -1], [10, 25, 50,100,250,500, "All"]],
            ...
            'serverMethod': 'post',
            'ajax': {
               'url': base_url + 'customer/customer/paysenz_CheckCreditCustomerList',
            "data": function ( data) {
         data.csrf_test_name = CSRF_TOKEN;
         data.customer_id =  $('#customer_id').val();
         data.customfiled =  $("select[name='customsearch[]']").val();
   ...
   var customer_id = $('#customer_id').val();
    ...
    var mydatatable = $('#paid_CustomerList').DataTable({ 
           ...
           'lengthMenu':[[10, 25, 50,100,250,500, -1], [10, 25, 50,100,250,500, "All"]],
            ...
            'serverMethod': 'post',
            'ajax': {
               'url':base_url + 'customer/customer/paysenz_CheckPaidCustomerList',
            "data": function ( data) {
         data.csrf_test_name = CSRF_TOKEN;
         data.customer_id =  $('#customer_id').val();
         data.customfiled =  $("select[name='customsearch[]']").val();
...
function supplierdelete(id) {
    ...
    if (r == true) {
        $.ajax({
            url: base_url + "supplier/supplier/paysenz_delete/"+id,
            type: "GET",
            success: function (r) {
               toastr["success"](r);
                setTimeout(function () {
