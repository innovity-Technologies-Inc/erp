...
class Warehouse_model extends CI_Model
{
    ...
    public function process_stock_movement($data)
    {
        ...
        if ($data['movement_data']['movement_type'] != 'IN') {
            // 🛡️ **CRITICAL FIX**: Ensure available_quantity never goes below 0
            $quantity = $data['movement_data']['quantity'];
            $batch_id = $data['movement_data']['batch_id'];

            // Get current available quantity
            $current_batch = $this->db->select('available_quantity')
                ->where('batch_id', $batch_id)
                ->get('batch_master')
                ->row();

            if ($current_batch) {
                $current_qty = floatval($current_batch->available_quantity);
                $new_qty = max(0, $current_qty - $quantity); // Prevent negative values

                // Update with safeguard
                $this->db->where('batch_id', $batch_id)
                    ->update('batch_master', ['available_quantity' => $new_qty]);

                log_message('info', "Stock Movement: Batch $batch_id - Current: $current_qty, Deducted: $quantity, New: $new_qty");
            }
        }
    ...
    /**
     * 🛡️ **CRITICAL MAINTENANCE FUNCTION**
     * Repairs all negative quantities in batch_master table
     * Call this on live server to fix existing negative values
     */
    public function repair_negative_quantities()
    {
        // Start transaction for data integrity
        $this->db->trans_start();

        // Get all batches with negative available_quantity
        $negative_batches = $this->db->select('id, batch_id, available_quantity')
        ...
        foreach ($negative_batches as $batch) {
            // Set all negative quantities to 0
            $this->db->where('id', $batch->id)
                ->update('batch_master', ['available_quantity' => 0]);

            log_message('warn', "Repaired Batch #{$batch->id} ({$batch->batch_id}): Changed available_quantity from {$batch->available_quantity} to 0");
            $repaired_count++;
        }

        // Also repair negative total_quantity
        $this->db->where('total_quantity <', 0)
            ->update('batch_master', ['total_quantity' => 0]);

        $this->db->trans_complete();

        if ($this->db->trans_status() === FALSE) {
            log_message('error', 'Failed to repair negative quantities');
            return false;
        }

        log_message('info', "Successfully repaired $repaired_count batches with negative quantities");
    ...
    public function get_warehouse_stock_by_product($product_id)
    {
        $this->db->select('w.id as warehouse_id, w.name as warehouse_name, bm.batch_id, bm.available_quantity as qty');
        $this->db->from('batch_master bm');
        $this->db->join('warehouse w', 'w.id = bm.warehouse_id', 'left');
        $this->db->where('bm.product_id', $product_id);
        $this->db->where('bm.available_quantity >', 0); // Optional: filter non-zero
        $this->db->order_by('w.id');
