...
{
    // Get all customers for merchant dropdown
    public function get_all_customers()
    {
        return $this->db->select('customer_id as id, customer_name as name')
    ...
    public function getSalesReportList($postData = null)
    {
        ...
        $merchant_id = $this->input->post('merchant_id'); // ✅ Get merchant_id filter
        ...
        $sql = "
            ...
            PaymentDetails AS (
                ...
                WHERE t.Debit > 0  
                GROUP BY i.invoice_id, i.date, i.total_amount, i.total_discount, i.paid_amount, i.due_amount, c.customer_name, i.customer_id
            )
            SELECT * FROM (
                SELECT 
                    i.invoice_id,
        ...
        // ✅ Apply Merchant/Customer Filter
        if (!empty($merchant_id)) {
            $sql .= " AND customer_id = '" . $this->db->escape_str($merchant_id) . "'";
        }
