  ...
  var report = $("#reportlist").DataTable({
    ...
    ajax: {
      ...
      data: function (data) {
        ...
        data.merchant_id = $("#merchant_id").val(); // ✅ Add merchant_id filter
        data.csrf_test_name = csrf_test_name;

        // ✅ Log search value for debugging
        console.log("🔎 Search Term Sent:", data.search?.value || "(empty)");
        console.log("🛍️ Merchant ID Sent:", data.merchant_id || "(empty)");
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      try {
        ...
        $(api.column(".total_discount").footer()).html(
          sumColumn("total_discount")
        );
        $(api.column(".payable_amount").footer()).html(
          sumColumn("payable_amount")
        );
